...
while (1)
      ...
      // pulsa per detik = data * 100
      // rps encoder = pulsa per detik / 16
      // rps shaft motor = rps encoder / 17.3
      // rpm shaft motor = rps shaftmotor * 60
      LCD_GotoXY(13,0); LCD_Angka3(dSpeedKa); LCD_Data(' ');
      LCD_GotoXY(7,1); LCD_Angka4((((dSpeedKa * 100.0)/16)/17.3)*60); LCD_TextF(" RPM");
      }
}
