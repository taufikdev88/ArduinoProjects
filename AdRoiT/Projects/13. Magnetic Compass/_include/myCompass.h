...
class myCompass {
    public:
        ...
        float   TiltCompensatedHeading(void);
        float   SimpleHeading(void);
...
// Start sampling pembacaan data magneto meter (melalui interupsi timer 1)
// lihat file "mySystem.h"
void    myCompass::Start(void)
{   myFlag.CompassStart = 1;
}

// Stop sampling pembacaan data magneto meter (melalui interupsi timer 1)
// lihat file "mySystem.h"
void    myCompass::Stop(void)
...
float   myCompass::TiltCompensatedHeading(void)
{
    if( (imu.Roll < 45 && imu.Roll > -45) && (imu.Pitch < 45 && imu.Pitch > -45))
    {
        ...
        float   Xh = Mx*cosRoll+Mz*sinRoll;
        float   Yh = Mx*sinPitch*sinRoll + My*cosPitch - Mz*sinPitch*cosRoll;
        return  (atan2(Yh,Xh)*RAD2DEG);
    }
    else    {   return 0;   }
...
float   myCompass::SimpleHeading(void)
{
    #if PosisiBoard == TIDUR
    return  (atan2(My, Mx)*RAD2DEG);
    #else
    return  (atan2(Mz, Mx)*RAD2DEG);
    #endif // PosisiBoard
}
...
{   if(TestConnection())
    {   // write CONFIG_A register
        ...
        i2c.Write((HMC5883L_AVERAGING_4<<5)|(HMC5883L_RATE_30<<2)|HMC5883L_BIAS_NORMAL);
        i2c.Stop();
...
void myCompass::ReadMagnetoMeter(void) {
    Data16Bit   Comp;
    if(myCompass::mode == HMC5883L_MODE_CONTINUOUS )
    {   i2c.Start(HMC5883L_ADDR | I2C_WRITE);
        i2c.Write(HMC5883L_RA_DATAX_H);
        i2c.Stop();
    }
    else if (myCompass::mode == HMC5883L_MODE_SINGLE )
    {   SetMode(HMC5883L_MODE_SINGLE);
    }
    i2c.Start_Wait(HMC5883L_ADDR | I2C_READ);
