...
{   // kalibrasi Hard Iron Distortion
    ...
    while (!Tombol1)
    {   ReadMagnetoRaw();
        if(Mx < magMin[xAxis])      magMin[xAxis] = Mx;
        else if(Mx > magMax[xAxis]) magMax[xAxis] = Mx;
        if(My < magMin[yAxis])      magMin[yAxis] = My;
        else if(My > magMax[yAxis]) magMax[yAxis] = My;
        if(Mz < magMin[zAxis])      magMin[zAxis] = Mz;
        else if(Mz > magMax[zAxis]) magMax[zAxis] = Mz;
