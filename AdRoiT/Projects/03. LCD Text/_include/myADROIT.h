...
Code    : Program Gerakan Robot AdRoiT V1
*********************************************************/
...
#endif // UsePIDmotor
    private:
        uint8_t CekSensor(uint8_t WarnaGaris, uint8_t WarnaLantai);
        ...
        uint8_t InvertSensor;       // variabel boolean untuk membalik atau tidak data pembacaan sensor, saat background lebih terang, data di invert
...
void    myAdroit::MajuPWM(uint16_t MotorKiri, uint16_t MotorKanan)
{
        ...
        myFlag.PIDMotorOn = 0;
    #endif // UsePIDmotor
    PwmM1 = MotorKiri;  M1_CCW;
    PwmM2 = MotorKanan; M2_CW;
};
...
void    myAdroit::MundurPWM(uint16_t MotorKiri, uint16_t MotorKanan)
{
        ...
        myFlag.PIDMotorOn = 0;
    #endif // UsePIDmotor
    PwmM1 = MotorKiri;  M1_CW;
    PwmM2 = MotorKanan; M2_CCW;
};
...
{   roda.dMotor1 = MotorKiri;    M1_CCW;
    roda.dMotor2 = MotorKanan;   M2_CW;
    myFlag.PIDMotorOn = 1;
};

void    myAdroit::MundurPID(uint8_t MotorKiri, uint8_t MotorKanan)
{   roda.dMotor1 = MotorKiri;    M1_CW;
    roda.dMotor2 = MotorKanan;   M2_CCW;
    myFlag.PIDMotorOn = 1;
};
...
{   float Error, U, ErrorSpeed;
    static float LastError=0, iError=0, iErrorSpeed=0, Top=3, CenterB=0;//, LastErrorSpeed=0;
    ...
    if(SpeedB!=0)
    {   if(++SpeedSampling==2)
        {   cSpeed = (int8_t)(Rotary1 - roda.Enkoder1)+ (int8_t)(Rotary2-roda.Enkoder2);
            ErrorSpeed = SpeedB - cSpeed;
            Rotary1 = roda.Enkoder1;
            Rotary2 = roda.Enkoder2;
...
{       myScanGaris::SensorGaris = CekSensor(WarnaGaris , WarnaLantai);
}
...
{       myScanGaris::SensorPertigaan = CekSensor(WarnaGaris , WarnaLantai);
}

// cek warna sensor yang diaktifkan berdasarkan kombinasi warna lantai dan warna garis
uint8_t myScanGaris::CekSensor(uint8_t WarnaGaris, uint8_t WarnaLantai)
{   //uint8_t WarnaSensor = SensorMerah;
    switch  (WarnaLantai)
    {   case    Hitam:      // jika warna laintai hitam
            myScanGaris::InvertSensor = 0;
...
{   uint8_t dSensor;
    ...
    if(myScanGaris::ArahGerak==GerakMundur) TopSpeed = - TopSpeed;
            ...
            if  (myScanGaris::InvertSensor) dSensor=~dSensor;
...
{   //int16_t Counter1, Counter2;
    uint8_t xCounter;
    roda.Enkoder1 = 0;    roda.Enkoder2 = 0;  // reset kondisi enkoder (cara ini tidak bisa dihunakan apabila digunakan juga metode odometri pada saat yang sama)
    if (myFlag.FlagBelok)   LastSpeed = StartSpeed;
    if(ArahGerak == GerakMaju)
    {   while((roda.Enkoder1 + roda.Enkoder2)<Counter)
...
{   SensorOffSet = 3;
    ...
    #else
        myScanGaris::BelokKananGarisPWM(((uint16_t)PidMotorKiri)<<4, ((uint16_t)PidMotorKanan)<<4);
    #endif // UsePIDmotor
    SensorOffSet = 0;
}
...
{   SensorOffSet = -3;
    ...
    #else
        myScanGaris::BelokKiriGarisPWM(((uint16_t)PidMotorKiri)<<4, ((uint16_t)PidMotorKanan)<<4);
    #endif // UsePIDmotor
    SensorOffSet = 0;
}
