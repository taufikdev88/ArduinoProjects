...
#define OUR_MAC "24:6f:28:aa:87:8e"

#define OTHER_MAC1 "30:ae:a4:97:ec:d2"
#define OTHER_MAC2 "24:62:ab:e6:4d:aa"
#define OTHER_MAC3 "234"
#define OTHER_MAC4 "435"
...
class MyAdvertisedDeviceCallbacks: public BLEAdvertisedDeviceCallbacks {
  void onResult(BLEAdvertisedDevice advertisedDevice) {
    ...
    if(DEVICE_MAC == OTHER_MAC1){
      DEVICE_RSSI1 = DEVICE_RSSI;
      digitalWrite(buzzer, 1);
    } else if(DEVICE_MAC == OTHER_MAC2){
      DEVICE_RSSI2 = DEVICE_RSSI;
      digitalWrite(buzzer, 1);
    } else if(DEVICE_MAC == OTHER_MAC3){
      DEVICE_RSSI3 = DEVICE_RSSI;
      digitalWrite(buzzer, 1);
    } else if(DEVICE_MAC == OTHER_MAC4){
      DEVICE_RSSI4 = DEVICE_RSSI;
      digitalWrite(buzzer, 1);
    } else {
      digitalWrite(buzzer, 0);
    }

    if(digitalRead(buzzer) == 1){ // cek jika memang ada yang dekat dan ada user tersambung
      StaticJsonDocument<256> doc;
      mode = "la";
      doc["mode"] = mode;
      doc["mac"] = ourDeviceMac;
      doc["data"]["r1"] = String(DEVICE_RSSI1);
      doc["data"]["r2"] = String(DEVICE_RSSI2);
      doc["data"]["r3"] = String(DEVICE_RSSI3);
      doc["data"]["r4"] = String(DEVICE_RSSI4);
      char datatemp[120] = {0};
...
class MyCallbacks: public BLECharacteristicCallbacks {
  void onWrite(BLECharacteristic *pCharacteristic){
    ...
    StaticJsonDocument<256> doc;
    if(mode == "i") mode = "ia";
    else if(mode == "h") mode = "ha";
    else if(mode == "o") mode = "oa";
...
void setup() {
  ...
  BLEDevice::init("UBAYA SoDis Master");
  ...
  Serial.println(F("Ubaya Sodis Master"));
...
void loop() {
  ...
  if((unsigned long) millis()-scanTiming >= 60000){
